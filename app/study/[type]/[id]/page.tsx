...
export default function StudyPage({ params }: { params: { type: string; id: string } }) {
  ...
  const handleGrammarClick = useCallback((grammar: string, timestamp?: string) => {
    setSelectedGrammar(grammar)
    if (timestamp && videoRef.current) {
      const seconds = convertTimestampToSeconds(timestamp)
      // Access the YouTube iframe's contentWindow
      const iframe = videoRef.current as HTMLIFrameElement & {
        contentWindow: { postMessage: (message: any, target: string) => void }
      }
  ...
  const handleWordClick = useCallback((word: string, timestamp?: string) => {
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <h1 className="mb-8 text-3xl font-bold">{type === "youtube" ? "YouTube Video Study" : "Song Lyrics Study"}</h1>
            ...
            <Card className="flex flex-col h-[600px]">
              <Tabs defaultValue="grammar" className="flex flex-col flex-1">
                ...
                <TabsContent value="grammar" className="flex-1 mt-0">
                  <Grammar level={id} onGrammarClick={handleGrammarClick} />
                </TabsContent>
                <TabsContent value="vocabulary" className="flex-1 mt-0">
                  <Vocabulary level={id} onWordClick={handleWordClick} />
                </TabsContent>
              </Tabs>
            </Card>
          )}
        ...
        <Card className="p-4 h-[600px]">
          <Tabs defaultValue="chat" className="h-full flex flex-col">
            ...
            <TabsContent value="chat" className="flex-1 mt-0">
              <Chat level={id} selectedGrammar={selectedGrammar} selectedWord={selectedWord} />
            </TabsContent>
